...
const commands =
  [
    ...
    // fail-exit no: удаление устаревшего чанка иногда даёт 550 «No such file»
    // (чанк уже пропал/перезалит) — это косметика, НЕ должна валить весь деплой
    // (новые чанки всё равно зальются; индекс пересобирается каждый прогон).
    "set cmd:fail-exit no",
